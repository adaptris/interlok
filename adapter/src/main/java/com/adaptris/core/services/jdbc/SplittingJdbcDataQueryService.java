...
/**
 * Perform a triggered JDBC query and the results of this query will be split out as separate messages or batches of messages.
 * 
 * @config splitting-jdbc-data-query-service
 * 
 * @author gdries
 */
@XStreamAlias("splitting-jdbc-data-query-service")
@AdapterComponent
@ComponentProfile(summary = "Query a database and store the results in the message", tag = "service,jdbc")
@DisplayOrder(order = {"connection", "statement", "statementParameters", "resultSetTranslator", "producer", 
    "parameterApplicator", "namespaceContext", "xmlDocumentFactoryConfig"})
public class SplittingJdbcDataQueryService extends JdbcService {
  ...
  @AutoPopulated
  @Valid
  private SplittingResultSetTranslator resultSetTranslator;
  @NotNull
  @AutoPopulated
  @Valid
  ...
  public SplittingJdbcDataQueryService() {
    setStatementParameters(new StatementParameterList());
    setResultSetTranslator(new SplittingXmlPayloadTranslator());
  ...
  public void doService(AdaptrisMessage msg) throws ServiceException {
    ...
    try {
      ...
      initXmlHelper(msg);
      System.err.println(msg.getObjectMetadata());
      ...
      Iterable<AdaptrisMessage> outputMessages = resultSetTranslator.translate(result);
  ...
  private void initXmlHelper(AdaptrisMessage msg) throws CoreException {
    ...
    msg.getObjectMetadata().put(JdbcDataQueryService.KEY_DOCBUILDER_FAC, builder);
    if (containsXpath(getStatementParameters())) {
      msg.getObjectMetadata().put(JdbcDataQueryService.KEY_XML_UTILS, XmlHelper.createXmlUtils(msg, namespaceCtx, builder));
    }
    if (namespaceCtx != null) {
      msg.getObjectMetadata().put(JdbcDataQueryService.KEY_NAMESPACE_CTX, namespaceCtx);
    }
  }

  private void destroyXmlHelper(AdaptrisMessage msg) {
    msg.getObjectMetadata().remove(JdbcDataQueryService.KEY_XML_UTILS);
    msg.getObjectMetadata().remove(JdbcDataQueryService.KEY_NAMESPACE_CTX);
  }
   ...
   */
  public SplittingResultSetTranslator getResultSetTranslator() {
   ...
   */
  public void setResultSetTranslator(SplittingResultSetTranslator outputTranslator) {
