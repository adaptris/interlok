 ...
 * Generally speaking, this is not encouraged, as you are now keeping configuration in 2 separate locations (both JNDI and adapter config).
 * The ConnectionFactory should be configured in JNDI with all the settings that are required for each connection.
 * </p>
 *
 * <p>
 * As the name suggests, this is a very simple implementation, primitive values are supported along with strings, but not objects. Every
 * fieldname referenced is expected to have an associated method set[fieldname] which has a single parameter; the match for which is
 * case-insensitive. If you have more more complex requirements then you will have to write your own implementation of
...
public class SimpleFactoryConfiguration implements ExtraFactoryConfiguration {
   ...
   * <p>
   * The key portion of the underlying {@link KeyValuePair} should match the name of the underlying ConnectionFactory field, the value is
   * the parameter to the associated setter. Note that only primitive types are supported (long, string, boolean, int, float, double).
   * </p>
   * <p>
   * If, for instance, you are looking up a Aurea SonicMQ connection factory from JNDI, and you wished to change the ConnectID field, then
   * the following configuration would be appropriate.
   *
   * <pre>
   * {@code
   *   <properties>
