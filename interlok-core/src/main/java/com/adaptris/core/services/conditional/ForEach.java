...
/**
* A for-each implementation that iterates over the payloads in a multi-payload message {@link MultiPayloadAdaptrisMessage}. For each
* payload then execute the given service (list). The default is to use a single thread to iterate over the payloads, but a thread pool can
...
* @since 3.10
*/
@JacksonXmlRootElement(localName = "for-each-payload")
@XStreamAlias("for-each-payload")
@AdapterComponent
@ComponentProfile(summary = "Runs the configured service/list for each multi-payload message payload.", tag = "for,each,for each,for-each,then,multi-payload", since = "3.10.0")
...
*
* If set to 0 then as many threads as there are payloads will be used.
*
* @param threadCount
...
message.getPayloadMessageHeaders(id).entrySet().forEach(e -> each.addMessageHeader(e.getKey(), e.getValue()));
