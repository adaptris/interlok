...
public class ForEachTest extends ConditionalServiceExample
{
  ...
  @Test
  public void testForEachWithPayloadMetadata() throws Exception {
    MultiPayloadAdaptrisMessage message = (MultiPayloadAdaptrisMessage) new MultiPayloadMessageFactory().newMessage("bacon", PAYLOAD_1);
    message.addPayload("cupcake", PAYLOAD_2);
    message.addPayloadMessageHeader("bacon", "key", "baconValue");
    message.addPayloadMessageHeader("cupcake", "key", "cupcakeValue");
    forEach.doService(message);
    assertEquals(Integer.valueOf(1), forEach.getThreadCount());
    verify(mock, times(2)).doService(any(AdaptrisMessage.class));
  }
