...
import com.adaptris.ftp.FtpClient;
...
public class TestFtp extends FtpCase {
  ...
  private static final Pattern LIST_DIR_PATTERN = Pattern
      .compile(LIST_DIR_FULL);
  ...
  public void testBug1924() throws Exception {
    if (areTestsEnabled()) {
      ...
      try {
        ...
        FtpClient client = new FtpClient("192.168.17.1", 21, 1);
        client.disconnect();
  ...
  public void testConnectWithTimeout() throws Exception {
    if (areTestsEnabled()) {
      ...
      try {
        ...
        FtpClient client = new FtpClient(config.getProperty(FTP_HOST), 21, 1);
  ...
  public void testGetLastModifiedWithTimezone() throws Exception {
    if (areTestsEnabled()) {
      ...
      try {
        ...
        FtpClient tzClient = (FtpClient) connectClientImpl();
  ...
  protected FileTransferClient connectClientImpl() throws Exception {
    FtpClient client = new FtpClient(config.getProperty(FTP_HOST));
    client.debugResponses(true);
